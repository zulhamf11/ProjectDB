...
class Products
{
	...
	public function addProduct($product_name,
		...
		if ($ext == "jpg" || $ext == "jpeg" || $ext == "png") {
			...
			if ($file['size'] > (1024 * 2)) {
				...
				if (move_uploaded_file($file['tmp_name'], $_SERVER['DOCUMENT_ROOT']."/ProjectDB/product_images/".$uniqueImageName)) {
	...
	public function addBrand($name){
		...
		if ($q->num_rows > 0) {
			...
			if ($q) {
				return ['status'=> 202, 'message'=> 'Product Type added Successfully'];
			}else{
				return ['status'=> 303, 'message'=> 'Failed to run query'];
			}
		}
	...
	public function deleteBrand($bid = null){
		if ($bid != null) {
			...
			if ($q) {
				return ['status'=> 202, 'message'=> 'Product Type removed'];
			}else{
				return ['status'=> 202, 'message'=> 'Failed to run query'];
			}
	...
	public function updateBrand($post = null){
		...
		if (!empty($brand_id) && !empty($e_brand_title)) {
			...
			if ($q) {
				return ['status'=> 202, 'message'=> 'Product Type Updated'];
			}else{
				return ['status'=> 202, 'message'=> 'Failed to run query'];
			}
